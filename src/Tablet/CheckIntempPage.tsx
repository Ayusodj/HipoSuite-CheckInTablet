...
`
  ...
  <div aria-hidden style={{ height: '12vh', width: '100%' }} />
      ...
      <div className="w-full max-w-3xl flex flex-col items-center gap-3">
        {/* Server configuration for Excel network server (tablet app) */}
        <div className="w-full panel-opaque rounded-lg shadow p-3">
          <div className="mb-2 font-medium">Configuración del servidor Excel (red)</div>
          <div className="grid grid-cols-1 gap-2">
            <input id="excel_server_url" placeholder="http://192.168.1.100:3000" defaultValue={localStorage.getItem('excel_server_url')||''} className="w-full border rounded px-3 py-2" />
            <input id="excel_server_path" placeholder="/shared/checkins.xlsx" defaultValue={localStorage.getItem('excel_server_path')||''} className="w-full border rounded px-3 py-2" />
            <div className="flex gap-2">
              <button className="px-3 py-2 bg-hipo-blue text-white rounded" onClick={() => { const u = (document.getElementById('excel_server_url') as HTMLInputElement).value; const p = (document.getElementById('excel_server_path') as HTMLInputElement).value; localStorage.setItem('excel_server_url', u); localStorage.setItem('excel_server_path', p); alert('Guardado'); }}>Guardar</button>
              <button className="px-3 py-2 border rounded" onClick={async () => { const u = (document.getElementById('excel_server_url') as HTMLInputElement).value; try { const res = await fetch(u+'/ping'); if (res.ok) alert('Servidor reachable'); else alert('No reachable: '+res.status); } catch(e){ alert('Error: '+e.message); } }}>Probar conexión</button>
            </div>
          </div>
        </div>
        {/* Languages row */}
