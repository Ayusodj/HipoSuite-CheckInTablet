...
const SettingsPage: React.FC<{ onClose: ()=>void }> = ({ onClose }) => {
  ...
  // Lock body scroll while settings modal is open to prevent background content from moving
  useEffect(() => {
    try {
  ...
  return (
    <div className="fixed inset-0 z-60 p-6 flex items-center justify-center bg-black/70">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl p-6 max-h-[85vh] overflow-y-auto overscroll-contain" style={{ WebkitOverflowScrolling: 'touch' }}>
        <h2 className="text-lg font-semibold mb-4">Ajustes tablet</h2>

        <div className="space-y-4">
          {/* Connection accordion */}
          <div>
            <button className="w-full text-left p-3 bg-gray-100 rounded" onClick={() => setConnOpen(!connOpen)}>
              <div className="flex justify-between items-center">
                <span className="font-medium">Conexión</span>
                <span className="text-sm text-gray-600">{connOpen ? '▾' : '▸'}</span>
              </div>
            </button>
            {connOpen && (
              <div className="p-3 space-y-3">
                ...
                <div>
                  ...
                  <input className="w-full border rounded px-3 py-2" value={path} onChange={e=>{ const v = e.target.value; setPath(v); try { const isUNC = v.startsWith('\\\\') || v.startsWith('\\') || v.startsWith('smb://') || v.startsWith('/') || /^[A-Za-z]:\\/.test(v); setPathValid(!!v ? isUNC : null); } catch { setPathValid(null); } }} placeholder="\\HIPOTELS.PRI\cmp\Recepcion\checkins.xlsx or /srv/shared/checkins.xlsx" />
          ...
          {/* SMB / Security accordion */}
          <div>
            <button className="w-full text-left p-3 bg-gray-100 rounded" onClick={() => setSmbOpen(!smbOpen)}>
              <div className="flex justify-between items-center">
                <span className="font-medium">SMB / Seguridad</span>
                <span className="text-sm text-gray-600">{smbOpen ? '▾' : '▸'}</span>
              </div>
            </button>
            {smbOpen && (
              <div className="p-3 space-y-3">
          ...
          <div>
            <button className="w-full text-left p-3 bg-gray-100 rounded" onClick={() => setAppearanceOpen(!appearanceOpen)}>
              <div className="flex justify-between items-center">
                <span className="font-medium">Apariencia</span>
                <span className="text-sm text-gray-600">{appearanceOpen ? '▾' : '▸'}</span>
              </div>
            </button>
            {appearanceOpen && (
              <div className="p-3 space-y-3">
          ...
          {/* Utilities accordion */}
          <div>
            <button className="w-full text-left p-3 bg-gray-100 rounded" onClick={() => setUtilsOpen(!utilsOpen)}>
              <div className="flex justify-between items-center">
                <span className="font-medium">Utilidades</span>
                <span className="text-sm text-gray-600">{utilsOpen ? '▾' : '▸'}</span>
              </div>
            </button>
            {utilsOpen && (
              <div className="p-3 space-y-3">
